...
const VerifyEmail = ({ email, onSuccess }: VerifyEmailProps) => {
  ...
  const handleResendOtp = async () => {
    try {
      setIsResending(true);
      const userData = useAuthStore.getState().signupData as {
        FirstName?: string;
        LastName?: string;
      };
      const fullName =
  ...
  return (
    <section>
      <h2 className="text-[29px]/[auto] text-[#737373] font-semibold mb-6">
        Verify your email
      </h2>
      <p className="text-[15px] font-medium mb-[30px]">
        We've sent an OTP message to{" "}
        <span className="text-[#F9E8CD]">{email}</span>. Enter it to activate
        your Clark account and start learning.
      </p>
      <Form {...form}>
