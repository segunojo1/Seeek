...
export interface ImageScanRiskItem {
  ailment_or_side_effect: string;
  trigger: string;
  severity: string;
}

export interface ImageScanRecommendation {
...
export interface ImageScanResponse {
  response: {
    ...
    personalized_recommendations: ImageScanRecommendation[];
...
export class ProductService {
  ...
  public async scanImage(
    ...
    try {
      ...
      console.warn("Unexpected scan response shape:", raw);
  ...
  public async analyzeQrCode(
    ...
    try {
      ...
      const raw = response.data;
      if (raw && typeof raw === "object") {
        return { data: raw as QrCodeAnalysisResponse, error: null };
      }

      console.warn("Unexpected barcode response shape:", raw);
