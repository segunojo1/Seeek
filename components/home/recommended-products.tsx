...
const RecommendedProducts = () => {
  const { recommendedMeals, fetchRecommendedMeals, isLoading } =
    useProductsStore();
  ...
  const validMeals = recommendedMeals
    .filter((meal) => meal.meal_name && !/^[^a-zA-Z0-9]+$/.test(meal.meal_name))
    .slice(0, 5);
      ...
      <div className="flex gap-4 overflow-x-auto pb-4">
        {validMeals.length > 0 ? (
          ...
          <Card className="text-[#525252] dark:text-[#A3A3A3] bg-white dark:bg-[#2C2C2C] rounded-[10px] p-4 w-full text-center">
            <p>
              No meal recommendations available. Complete your profile for
