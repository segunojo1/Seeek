...
import MealCard, {
  SpotlightedMeal,
  TopInsight,
} from "@/components/explore/meal-card";
            ...
            onKeyDown={(e) => {
              if (e.key === "Enter") {
                ...
                if (searchTerm) {
                  ...
                  router.push(
                    `/meal/${encodeURIComponent(searchTerm.toLowerCase())}`,
                  );
                  // if (matchingMeal) {
                  // }
                }
      ...
      {activeTab === "discover" && (
        <>
          <div className="flex gap-5 flex-col lg:flex-row">
            {featuredMeal ? (
              <SpotlightedMeal meals={recommendedMeals} />
            ) : (
              ...
              {isLoading ? (
                Array(3)
                  ...
                  .map((_, i) => (
                    <Skeleton
                      key={i}
                      className="w-full h-[300px] rounded-[20px] bg-white"
                    />
                  ))
              ) : recommendedMeals.length > 0 ? (
                recommendedMeals
                ...
                {Array(3)
                  ...
                  .map((_, i) => (
                    <Skeleton
                      ...
                      className="w-full h-64 rounded-[20px]"
                    />
                  ))}
              </div>
            ) : blogs.length > 0 ? (
